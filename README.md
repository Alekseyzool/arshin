...
- VRI sync аналогично может уменьшать размер страницы до `VRI_MIN_ROWS` (по умолчанию `10`), если `vri_cursor` начинает получать `429`, `5xx` или таймауты на больших страницах.
- Если конкретный `cursorMark` у VRI стабильно падает даже на `VRI_MIN_ROWS`, загрузчик автоматически переключается на `start`-пагинацию с уже пройденного смещения и не роняет весь день из-за одной битой страницы.
- HTTP-клиент соблюдает реальный лимит `FGIS_RPS` между стартами запросов; если FGIS отвечает `429`, клиент использует `Retry-After` или экспоненциальный backoff. Если `429` продолжаются, снижайте `FGIS_RPS` до `0.2-0.5`.
