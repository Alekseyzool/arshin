...
- Для медленных ответов FGIS можно увеличить `HTTP_TIMEOUT`; MIT sync также может автоматически уменьшать размер страницы до `MIT_MIN_ROWS`, если запрос страницы падает по таймауту или retryable HTTP-ошибке.
- VRI sync аналогично может уменьшать размер страницы до `VRI_MIN_ROWS`, если `vri_cursor` начинает получать `429` или таймауты на больших страницах.
