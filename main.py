...
from fgis_clickhouse.ui_helpers import ch_connect_from_sidebar
...
def run_analytics_tab(ch: Optional[CH]) -> None:
    ...
    if ch is None:
        st.info("Сначала подключитесь к ClickHouse.")
        return

    col1, col2, col3 = st.columns(3)
    with col1:
        manufacturer_term = st.text_input("Производитель (подстрока)", key="ana_manufacturer")
